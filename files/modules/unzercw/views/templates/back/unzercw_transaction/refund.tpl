...
{if $transaction->getTransactionObject()->isPartialRefundPossible()}
	<form method="POST" class="unzercw-line-item-grid" id="refund-form">
		...
		<table class="table table-striped table-condensed table-hover table-bordered">
			...
			<tfoot>
				<tr>
					<td colspan="6" class="text-right">{lcw  mod='unzercw' s='Total Refund Amount'}:</td>
		...
		{if $transaction->getTransactionObject()->isRefundClosable()}
			<div class="closable-box">
				<label for="close-transaction">{lcw  mod='unzercw' s='Close transaction for further refunds'}</label>
				<input id="close-transaction" type="checkbox" name="close" value="on" />
			</div>
		{/if}
		
		<div class="text-right">
			<input type="submit" class="button btn btn-success" value="{lcw  mod='unzercw' s='Refund'}" />
		</div>
	</form>
{/if}
