 ...
 *
 */
class UnzerCwIframeModuleFrontController extends ModuleFrontController {
	public $ssl = true;
...
require_once 'UnzerCw/Adapter/IframeAdapter.php';
		...
		$wrapper = UnzerCw_Util::getShopAdapterByPaymentAdapter(
				UnzerCw_Util::getAuthorizationAdapter($transaction->getTransactionObject()->getAuthorizationMethod()));

		if (!($wrapper instanceof UnzerCw_Adapter_IframeAdapter)) {
			throw new Exception("Expect 'UnzerCw_Adapter_IframeAdapter'.");
		}
		...
		$this->setTemplate('module:unzercw/views/templates/front/iframe.tpl');
	}
}
