...
/**
 * UnzerCw_DirectDebitsSepa
 *
 * This class defines is the module class for the
 * payment method "DirectDebitsSepa".
 *
 * @author customweb GmbH
 */
class UnzerCw_DirectDebitsSepa extends PaymentModule implements UnzerCw_IPaymentMethod {
	/**
	 *
	...
	public $name = 'unzercw_directdebitssepa';
	public $paymentMethodName = 'directdebitssepa';
	public $paymentMethodDisplayName = 'Sepa Direct Debits';
			...
			0 => array(
 			...
 			4 => array(
				'name' => 'PLACEHOLDER_SIZE',
 				'label' => $this->l("Element Size"),
 				'desc' => $this->l("How should elements from
						 be loaded? With narrow elements
						the element label is displayed by the store, with wide elements it
						is
						loaded via javascript by . The
						input elements are always loaded from
						.
					"),
 				'default' => 'narrow',
 				'type' => 'select',
 				'options' => array(
					'query' => array(
						0 => array(
							'id' => 'wide',
 							'name' => $this->l("Wide (label from
							)
						"),
 						),
 						1 => array(
							'id' => 'narrow',
 							'name' => $this->l("Narrow (label from shop)"),
 						),
 					),
 					'name' => 'name',
 					'id' => 'id',
 			...
 			7 => array(
				'name' => 'MERCHANT_NAME',
 				...
 				'desc' => $this->l("Here you can configure the merchant name which is
						displayed as part of the mandate text.
					"),
 				'default' => '',
 				'type' => 'text',
 			),
...
$fields = array_merge($this->getFormFieldsInner(), $fields);
	...
	public function installMethodSpecificConfigurations(){
		...
		$this->getConfigApi()->updateConfigurationValue('PLACEHOLDER_SIZE', 'narrow');
		$this->getConfigApi()->updateConfigurationValue('SEND_BASKET', 'no');
		$this->getConfigApi()->updateConfigurationValue('SEND_CUSTOMER', 'no');
		$this->getConfigApi()->updateConfigurationValue('MERCHANT_NAME', '');
	...
	public function uninstallMethodSpecificConfigurations(){
		...
		$this->getConfigApi()->removeConfigurationValue('PLACEHOLDER_SIZE');
		$this->getConfigApi()->removeConfigurationValue('SEND_BASKET');
		$this->getConfigApi()->removeConfigurationValue('SEND_CUSTOMER');
		$this->getConfigApi()->removeConfigurationValue('MERCHANT_NAME');
